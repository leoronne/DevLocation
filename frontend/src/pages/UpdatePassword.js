...
export default function UpdatePassword() {
      ...
      useEffect(() => {
            if (params.getAll('token')[0] === '' || params.getAll('token')[0] === undefined || params.getAll('passtoken')[0] === '' || params.getAll('passtoken')[0] === undefined) {
                  notify(`Error while identifying Dev!`, '⚠️', 'error', 'top-right');
                  history.push('/');
            }
            api.get('dev/validpasswordtoken', {
                  params: {
      ...
      async function handleUpdatePassword() {
            ...
            if (await inputValidation()) {
                  ...
                  await api.post('dev/updatepassword', data,
                        {
                              headers: {
            ...
            <div className='confirm-container'>
                  <section className='form'>
                        ...
                        <form>
                              ...
                              <br /> <br /> <br />
                              <Button disabled={loading} color='primary' className='button' type='button' onClick={handleUpdatePassword}>
                                    {loading ? <DotLoader
                                          css={`display: block;
                                                margin: 0 auto;
                                                border-color: red;
