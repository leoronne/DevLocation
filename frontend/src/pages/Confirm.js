...
export default function Confirm(props) {
      ...
      useEffect(() => {
            if (params.getAll('id')[0] === '' || params.getAll('id')[0] === undefined) {
                  notify(`Error while identifying Dev's ID!`, '⚠️', 'error', 'top-right');
                  history.push('/');
            }
            api.post('dev/confirm', {
      ...
      async function handleEmailVerification() {
            ...
            if (await inputValidation()) {
                  ...
                  await api.post('dev/sendemail', data)
                        .then((response) => {
            ...
            <div className='confirm-container'>
                  <section className='form'>
                        ...
                        <form>
                              ...
                              <Button disabled={loading} color='primary' className='button' type='button' onClick={handleEmailVerification}>
                                    {loading ? <DotLoader
                                          css={`
                                                      display: block;
                                                      margin: 0 auto;
